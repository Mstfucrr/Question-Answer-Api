...
const getAllAnswersByQuestion = asyncErrorWrapper(async (req, res, next) => {
    ...
    const question = await Question.findById(question_id).populate("answers"); // question_id'ye göre question'ı bul ve question'ın answers'larını getir
...
const deleteAnswer = asyncErrorWrapper(async (req, res, next) => {
    ...
    await Answer.findByIdAndRemove(answer_id)
        .then(async () => {
            ...
            return next(new CustomError("Answer couldn't be deleted", 400));
        })
...
const undoLikeAnswer = asyncErrorWrapper(async (req, res, next) => {
    ...
    return res.status(200)
        .json({
            success: true,
            message: "Undo like operation is successful"
        })
});
